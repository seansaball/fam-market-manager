...
**3,646 tests across 161+ test files** (v2.0.8; was 3,504 in v2.0.7, 3,387 in v2.0.6) covering: formula validation, match limits, returning customers, transaction adjustments, FMNP reports, market codes, device IDs, backups, schema migrations, settings import/export, cloud sync (including the multi-workstation invariant — every settings mutation triggers a sync, every cleanup gates by device_id), auto-update, charge conversion, denomination validation and preservation through adjustments, the universal SNAP/Cash vendor binding policy, single-vendor multi-receipt allocation, the cap-bound split-orders defensive recommendation, photo storage (with atomic-write hardening), multi-photo workflows, photo deduplication with cache cleanup on void/delete/replace, FMNP sync integration, integer-cents boundary validation, three-way reconciliation (DB/Ledger/Sheets), automated UI testing (payment screen, workflows, market-day simulation), max-cap clamping, market day lifecycle guards, market-rename code-shift protection, adjustment edge cases including the denom-method safety gate, payment method CRUD safety, match-cap-aware charge input, end-to-end production readiness, v1.9.9 stress scenarios, audit-log coverage, and CSV-vs-DB export reconciliation.
