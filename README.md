...
- **Photo Receipt Capture** — Attach photos of FMNP checks and other collected scrip to External Payments entries, and check/receipt photos to payment transactions, with denominated photo slots
...
**3,873 tests across 173 test files** (v2.1.0; was 3,646 in v2.0.8, 3,504 in v2.0.7) covering: formula validation, match limits, returning customers, transaction adjustments, FMNP reports, market codes, device IDs, backups, schema migrations, settings import/export, cloud sync (including the multi-workstation invariant — every settings mutation triggers a sync, every cleanup gates by device_id), auto-update, charge conversion, denomination validation and preservation through adjustments, the universal SNAP/Cash vendor binding policy, single-vendor multi-receipt allocation, the cap-bound split-orders defensive recommendation, photo storage (with atomic-write hardening), multi-photo workflows, photo deduplication with cache cleanup on void/delete/replace, FMNP sync integration, integer-cents boundary validation, three-way reconciliation (DB/Ledger/Sheets), automated UI testing (payment screen, workflows, market-day simulation), max-cap clamping, market day lifecycle guards, market-rename code-shift protection, adjustment edge cases including the denom-method safety gate, payment method CRUD safety, match-cap-aware charge input, end-to-end production readiness, v1.9.9 stress scenarios, audit-log coverage, CSV-vs-DB export reconciliation, external payment entries (snapshot-config payout, the G4 config linter, EP-series invariants), the SNAP Settlement report, market-day and whole-month report filters, vendor verification marks, and standard money typing.
...
This runs four gates in sequence and halts on the first failure:

1. **Full pytest suite** (~140 s, 3,800+ tests)
...
4. **Randomized fuzz smoke** — `python -m scripts.fuzz_simulator` (5 seeds × 100 random actions; saves a JSON reproduction artifact on failure). Known state: a small set of expected-red seeds is documented in `CLAUDE.md` rule 4 (BUG-006) — CI deselects/soft-fails exactly those, annotated with the bug ID.
...
| Version | Summary |
|---------|---------|
| **v2.1.2** | **Collection variance + reporting/sync polish (July 2026).**  Schema v41 (unchanged — no migration).  **"Correct Amount Collected"** (ENH-008) — a new option under Adjust for when the amount actually charged (e.g. an EBT terminal keying error) differs from what was logged: it lowers the recorded amount to the real charge, **keeps the FAM match intact**, and books the difference as Unallocated Funds so SNAP totals match the bank deposit while the vendor total is untouched (surgical, engine-free, never touches the Layer 2A guard).  **ACH Enabled column** on the Vendor Reimbursement report (in-app + Sheets) — Yes/No per vendor from Settings → Vendors, backfills past months on next sync.  **Auto-sync keeps running after the market day closes** — data entered later (reconnecting the next day) flushes automatically, no manual sync.  **Quiet offline logging** — third-party connection-retry chatter suppressed and FAM's offline state logged once per episode (not per cycle), so a closed-day probe never inflates the Error Log.  Includes v2.1.1's sub-dollar denominations.  See `RELEASE_NOTES_v2.1.2.md`. |
| **v2.1.1** | **Sub-dollar denominations (July 2026).**  Schema v41 (unchanged).  Payment-method denominations can be set below $1.00 (e.g. JH Tokens at $0.50, $0.25 coupons; minimum $0.01) — previously the $1.00 floor silently snapped a typed $0.50 back to $1.00.  All denomination math was already exact integer cents, so smaller values flow through cleanly.  See `RELEASE_NOTES_v2.1.1.md`. |
| **v2.1.0** | **"External Payments Entry" release (June 2026).**  Schema v37 → **v41** through four additive migrations (v38 external-payments columns, v39 settlement-verified column, v40/v41 vendor-verification tables), each preceded by an automatic backup.  **The FMNP Entry screen generalizes to External Payments Entry** (ENH-002) — coordinators record ANY external-enabled method's scrip collected from vendors (FMNP, Food RX, JH Food Bucks, future methods); FMNP stays the pre-selected default and behaves exactly as before.  **Per-method external reimbursement** from each entry's snapshot config via the single payout module (`fam/utils/external_payout.py`, integer-cents, round-half-away-from-zero); every synced row carries a plain-English Reimbursement Basis.  New **"External Payment Entries"** sheet tab replaces the deprecated FMNP Entries tab (each market's rows move atomically with its own upgrade; no double-counting).  **SNAP Settlement report** (ENH-003) — one row per EBT terminal swipe with a working Verified checkbox, for matching the terminal's paper receipts.  **Market Day report filter** + whole-month quick pick (ENH-005) — every report tab narrows to exactly one market day.  **Vendor Reimbursement Verified column** (ENH-006) — per-scope verification marks for the end-of-market vendor walk and month-end pass; zero effect on reimbursement, never syncs.  **Standard money typing** (ENH-007) — every money box now types calculator-style (WYSIWYG, decimal key works), replacing the ATM-style penny ladder.  Food Bucks seed match corrected to 0% (ENH-001, fresh installs only).  UI polish: stable per-method chart colors, payment-methods donut with center total, dollar-formatted trend axis, app-font charts, help-walkthrough layout fixes.  **3,873 tests across 173 files.**  See `RELEASE_NOTES_v2.1.0.md`. |
| **v2.0.9** | **Vendor Reimbursement monthly split (May 2026).**  Schema v37 (unchanged — no migrations).  The Vendor Reimbursement Google Sheet becomes **one row per (market × vendor × month)** instead of one ever-growing all-time-cumulative row per vendor, so coordinators reconcile reimbursements month-over-month at a glance; the new sortable **Year-Month** column joins the row identity for upsert.  Sync-layer only — no booth-side behavior change, on-screen reports unchanged; the per-row math identity holds within every monthly row.  Old cumulative rows are replaced automatically on each device's first v2.0.9+ full sync.  See `RELEASE_NOTES_v2.0.9.md`. |
| **v2.0.8** | **Production release (May 2026)** consolidating the v2.0.7 hotfix + post-hotfix follow-ups into a single shipping version.  Schema v35 → **v37** through two forward migrations: v36 adds `customer_forfeit_cents` (Phase B token-value forfeit, schema persistence for the customer-side over-tender), v37 adds `user_capped` (the volunteer's per-row "this value is final" flag, schema-persisted so a Locked row survives draft save/restore + adjustment round-trips).  **Per-row ⚡ Auto-Distribute toggle** — green/grey states on each non-denom payment row; volunteer types or clicks ⚡ to lock; only one row at a time can be the Active overflow target (radio invariant enforced at row-add).  **Denomination-integrity in reports** — Vendor Reimbursement and Detailed Ledger now show the customer's **physical handout** (`customer_charged + forfeit`) in per-method columns, not the post-forfeit value; row reconciliation: Σ(method-cols) + FAM Match − Customer Forfeit + FMNP_External == Total Due.  **Customer Forfeit summary card** on Payment Screen + dedicated column in reports.  **FMNP Check Tracking page** gains an "All Market Days" filter for browsing the full FMNP history with date-range search; visible inline hint label appears next to the disabled Save button when "All Market Days" is selected.  **Auto-Distribute cap-deficit Pass 2 fallback** — match-cap deficits now redistribute to unmatched non-denom auto rows (Cash) when matched rows are locked, so the user-reported "SNAP locked, Cash auto, Auto-Distribute does nothing" scenario fills Cash correctly.  **Status-filter centralization** — `fam.models.transaction.ACTIVE_TX_STATUSES` + `active_tx_status_clause(alias)` helper eliminates the inline `status IN ('Confirmed', 'Adjusted')` duplication across 18+ sites; future reports use a single source of truth.  **Spinbox crash fix** — `NoScrollSpinBox` / `NoScrollDoubleSpinBox` no longer raise `ValueError: invalid literal for int() with base 10: ''` on Page Up / Page Down (the empty-string substring trap).  **AdjustmentDialog parity** — `_update_customer_impact` now propagates `user_capped` to the engine so the impact preview matches the actual save (no more "row mismatch" surprise at confirm).  **Sync watchdog** — 5-minute QTimer guard on the cloud sync button so a stuck sync state self-recovers.  Documentation: U14-U17 + R3, R5, R7 added to SYSTEM_INVARIANTS.md; FINANCIAL_FORMULA.md §3b.0 + §4a-c + §6.2 + §6.4 reflect the user-cap engine policy and denomination-integrity reconciliation; 3 new in-app help articles (`auto-distribute-toggle`, `customer-forfeit`, `fmnp-all-market-days`).  **3,646 tests across 161+ files**, +143 new tests since v2.0.7. |
| **v2.0.7** | **Hotfix release (May 2026).**  Schema v34 → **v35** through one forward migration that backfills SNAP and Cash bindings for every vendor.  **Headline policy: SNAP and Cash are universally accepted at every vendor by default and cannot be unassigned** — eliminates the silent-SNAP-onto-ineligible-vendor reproducer class entirely.  **Denomination preservation through adjustments** — engine snap-back + save-layer guard keep denom rows aligned to physical-token multiples through every code path.  **Adjustment safety gate for denominated transactions** — three-button dialog (Void Instead / Adjust Anyway / Cancel) when the txn includes Food Bucks / Food RX / FMNP; override path emits an `ADJUST_OVERRIDE` audit log entry.  **Single-vendor multi-receipt allocation** — Layer 2C and the save path now treat single-vendor orders with multiple receipts as a unit (proportional distribution to per-receipt remaining capacity).  **Vendor Reimbursement after voids** — aggregation correctly excludes voided lines.  **Photo dedup cache invalidation** on void / FMNP delete / FMNP photo replace.  **Cap-bound impossible-to-balance recommendation** — Layer 2A's mismatch dialog now detects the cap-bound + denom + non-denom-overshoot pattern and explicitly recommends splitting the customer's receipts into separate orders one method at a time; backed by a new `split-orders-when-stuck` help article and `ts-payment-screen-hard-block` troubleshooting flow.  **Layer 2B eligibility-blamed gate** — no more "X cannot accept SNAP" misfires when every vendor accepts SNAP.  **Save-path per-vendor method eligibility filter** — Phase 2 of the save path matches Layer 2C's simulation so save and validation stay in lock-step.  **Denomination-aware Auto-Distribute** — seed-row pass respects per-vendor eligibility for denom methods at vendor-binding time.  **Reverted (pinned absent):** an attempted `_auto_rebalance_non_denom` fought the engine's deterministic cap-aware Path B + Pass 4; the split-orders recommendation is the durable resolution for cap-bound scenarios.  **3,504 tests passing** across 65+ files.  See `RELEASE_NOTES_v2.0.7.md`. |
