...
class PaymentScreen(QWidget):
    ...
    def _build_ui(self):
        ...
        self.vendor_frame = QFrame()
        self.vendor_frame.setStyleSheet(f"""
            QFrame {{
                background-color: {WHITE};
                border: 1px solid {LIGHT_GRAY};
                border-radius: 8px;
        ...
        """)
        vendor_inner = QVBoxLayout(self.vendor_frame)
        vendor_inner.setSpacing(4)
        vendor_lbl = QLabel("Vendor Breakdown:")
        vendor_lbl.setStyleSheet("font-weight: bold;")
        vendor_inner.addWidget(vendor_lbl)
        ...
        # Ensure table has room for header + a few rows
        self.vendor_table.setMinimumHeight(100)
        vendor_inner.addWidget(self.vendor_table)

        layout.addWidget(self.vendor_frame)
    ...
    def _populate_vendor_summary(self):
        ...
        self.vendor_frame.setVisible(len(rows) > 0)
    ...
    def _update_summary(self):
        ...
        if entries:
            calc_entries = [
                {'method_amount': e['method_amount'], 'discount_percent': e['discount_percent']}
                for e in entries
            ]
            result = calculate_payment_breakdown(receipt_total, calc_entries)
    ...
    def _update_collection_list(self, entries, result):
        ...
        for i, li in enumerate(result['line_items']):
            ...
            if discount_amount > 0:
                text += f"  (FAM matches ${discount_amount:.2f})"
    ...
    def _confirm_payment(self):
        ...
        entries = [
            {'method_amount': it['method_amount'], 'discount_percent': it['discount_percent']}
            for it in items
        ]
        ...
        if result.get('line_items'):
            for i, li in enumerate(result['line_items']):
                ...
                if discount_amount > 0:
                    line += f"  (FAM matches ${discount_amount:.2f})"
        ...
        self._distribute_and_save_payments(items, receipt_total)

        snap_ref = self.snap_ref_input.text().strip() or None
        for t in self._order_transactions:
            if snap_ref:
                update_transaction(t['id'], snap_reference_code=snap_ref)
            confirm_transaction(t['id'], confirmed_by="Volunteer")
    ...
    def _distribute_and_save_payments(self, items, order_total):
        if order_total <= 0:
            return
        ...
        for t in self._order_transactions:
            ...
            for item in items:
                ...
                discount_pct = item['discount_percent_snapshot']
                discount_amount = round(method_amount * (discount_pct / 100.0), 2)
            ...
            save_payment_line_items(t['id'], txn_items)
